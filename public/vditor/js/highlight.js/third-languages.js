...
hljs.registerLanguage(
  ...
  (() => {
    ...
    var s = { className: 'number', begin: a, relevance: 0 },
      ...
      i = { className: 'string', begin: /\bhex'(([0-9a-fA-F]{2}_?)*[0-9a-fA-F]{2})?'/ },
      l = { className: 'string', begin: /\bhex"(([0-9a-fA-F]{2}_?)*[0-9a-fA-F]{2})?"/ }
    function d(e) {
    ...
    return (e) => {
      ...
      var t = { className: 'built_in', begin: s },
        i = c(e)
      return e.inherit(i, { keywords: a, contains: i.contains.concat([t]) })
    }
  })(),
)
...
hljs.registerLanguage(
  ...
  (() => {
    ...
    var s = { className: 'number', begin: a, relevance: 0 },
      ...
      i = { className: 'string', begin: /\bhex'(([0-9a-fA-F]{2}_?)*[0-9a-fA-F]{2})?'/ },
      t = { className: 'string', begin: /\bhex"(([0-9a-fA-F]{2}_?)*[0-9a-fA-F]{2})?"/ }
    function r(e) {
      return e.inherit(e.APOS_STRING_MODE, { begin: /(\bunicode)?'/ })
    }
    function l(e) {
    ...
    return (e) => {
      ...
      var l = n.map((e) => 'bytes' + e).join(' ') + ' ',
        ...
        y = e.inherit(e.TITLE_MODE, { begin: /[A-Za-z$_][0-9A-Za-z$_]*/, lexemes: C, keywords: p }),
        w = {
          className: 'built_in',
      ...
      return {
        ...
        contains: [
          ...
          x('block', 'blockhash coinbase difficulty gaslimit basefee number timestamp chainid'),
          x('tx', 'gasprice origin'),
          x('abi', 'decode encode encodePacked encodeWithSelector encodeWithSignature encodeCall'),
          x('bytes', 'concat'),
          x('string', 'concat'),
          f,
          {
...
hljs.registerLanguage(
  ...
  (() => {
    ...
    const e = {
      className: 'number',
      begin:
        '(-?)(\\b0[xX][a-fA-F0-9]+|(\\b\\d+(\\.\\d*)?([hHfFlL]?)|\\.\\d+)([eE][-+]?\\d+)?([hHfFlL]?))',
      relevance: 0,
    }
    return (r) => {
      let t = [
        ...
        a = []
      for (let e of 'bool double float half int uint min16float min10float min16int min12int min16uint'.split(
        ' ',
      ))
        for (let r of t) a.push(e + r)
      let s =
          'SV_Coverage SV_Depth SV_DispatchThreadID SV_DomainLocation SV_GroupID SV_GroupIndex SV_GroupThreadID SV_GSInstanceID SV_InnerCoverage SV_InsideTessFactor SV_InstanceID SV_IsFrontFace SV_OutputControlPointID SV_Position SV_PrimitiveID SV_RenderTargetArrayIndex SV_SampleIndex SV_StencilRef SV_TessFactor SV_VertexID SV_ViewportArrayIndex, SV_ShadingRate',
        o =
          'BINORMAL BLENDINDICES BLENDWEIGHT COLOR NORMAL POSITION PSIZE TANGENT TEXCOORD TESSFACTOR DEPTH SV_ClipDistance SV_CullDistance SV_DepthGreaterEqual SV_DepthLessEqual SV_Target SV_CLIPDISTANCE SV_CULLDISTANCE SV_DEPTHGREATEREQUAL SV_DEPTHLESSEQUAL SV_TARGET',
        n = o.split(' ')
      ...
      return {
        ...
        keywords: {
          ...
          type:
            a.join(' ') +
            ' Buffer vector matrix sampler SamplerState PixelShader VertexShader texture Texture1D Texture1DArray Texture2D Texture2DArray Texture2DMS Texture2DMSArray Texture3D TextureCube TextureCubeArray struct typedef',
          built_in:
